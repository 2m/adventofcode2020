...
object Computer extends IOApp {
  ...
  val program = commands
    .zipWithScan(Option.empty[Mask]) { case (mask, input) =>
      input match {
        case m: Mask => Some(m)
        case _       => mask
      }
    }
    .collect { case (c: Command, Some(m)) =>
      (c, m)
    }

  def decoderV1[F[_]]: Pipe[F, (Command, Mask), Map[BigInt, BigInt]] =
    _.fold(Map.empty[BigInt, BigInt]) { case (memory, (command, mask)) =>
  ...
  def decoderV2[F[_]]: Pipe[F, (Command, Mask), Map[BigInt, BigInt]] =
    _.fold(Map.empty[BigInt, BigInt]) { case (memory, (command, mask)) =>
      ...
      expand(Seq(modified.mkString)).foldLeft(memory) { case (memory, address) =>
        memory.updated(BigInt(address.mkString, 2), command.value)
      }
    }
